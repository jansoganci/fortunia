...
serve(async (req) => {
  ...
  try {
    ...
    // Call the consume_quota function
    const { data, error } = await supabaseClient.rpc('consume_quota', {
      p_user_id: activeUserId
    })

    if (error) {
      console.error('Error calling consume_quota:', error)
      return new Response(
        JSON.stringify({ error: 'Failed to consume quota', details: error.message }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    if (!quotaInfo.success) {
      return new Response(
        JSON.stringify({
          ...
          message: quotaInfo.message || 'Daily quota exceeded',
          quota_used: quotaInfo.quota_used || 3,
    ...
    return new Response(
      JSON.stringify({
        ...
        is_premium: quotaInfo.is_premium || false,
        message: quotaInfo.message || 'Quota consumed successfully'
      }),
      { 
        status: 200, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
